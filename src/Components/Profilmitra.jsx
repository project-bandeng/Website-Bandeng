      ...
      <nav className="col-md-2 d-none d-md-block sidebar rounded-4 pe-0 ps-4" style={{ backgroundColor: '#0F75BD' }}>
          ...
          <div className="position-sticky">
            ...
            <ul className="nav flex-column gap-2">
              <li className={`nav-item rounded-start-3 ${activeMenu === 'profil' ? 'active' : ''}`}>
                <a className={`nav-link text-primary  ${activeMenu === 'profil' ? 'active-link' : '/profil'}`} href="/profil" onClick={() => setActiveMenu('profil')}>
                  <i class="bi bi-person-fill me-2"></i>
                  PROFILE
                </a>
              </li>
              <li className={`nav-item ${activeMenu === 'products' ? 'active' : ''}`}>
                <a className={`nav-link text-white hovering-menu rounded-start-3 ${activeMenu === 'products' ? 'active-link' : '/crudproduk'}`} href="/crudproduk" onClick={() => setActiveMenu('products')}>
          ...
          <div className="row">
            <div className="col-md-3">
              <div className="card rounded-3">
                <img src={fotoMitra} alt="Profil Pengguna" className="card-img-top" />
                <div className="card-body d-flex flex-column align-items-center">
                  <label className="btn btn-primary me-2 mt-1" htmlFor="fotoprofil">
                    UBAH FOTO PROFIL
                  </label>
                  <input type="file" name="fotoprofil" id="fotoprofil" className="visually-hidden" accept="image/*" onChange={handleOnChangeFoto} />
                  <button className="btn btn-primary me-2 mt-2">UBAH KATA SANDI</button>
                </div>
              </div>
            </div>
                  ...
                  {editMode ? (
                    <>
                      ...
                      <div className="mb-3">
                        ...
                        <input type="date" className="form-control" id="tgl_lahir" value={tgl_lahir} onChange={(e) => setTgl(e.target.value)} />
                      </div>
                      <div className="mb-3">
                        <label htmlFor="jenis_kel" className="form-label">
                          Jenis Kelamin
                        </label>
                        <input type="text" className="form-control" id="jenis_kel" value={jenis_kel} onChange={(e) => setKelamin(e.target.value)} />
                      </div>
                      <div className="mb-3">
                        <label htmlFor="email" className="form-label">
                          Email Pengguna
                        </label>
                        <input disabled type="text" className="form-control" id="email" value={email} onChange={(e) => setEmail(e.target.value)} />
                      </div>
                      <div className="mb-3">
                        <label htmlFor="no_hp" className="form-label">
                          No. Handphone
                        </label>
                        <input type="text" className="form-control" id="no_hp" value={no_hp} onChange={(e) => setNomer(e.target.value)} />
                      </div>
                    ...
                    <div className='d-flex flex-column'>
                      ...
                      <button className="btn btn-primary mt-4 align-self-center" onClick={handleEdit}>
                        Edit Profil
                      </button>
                    </div>
                  )}
